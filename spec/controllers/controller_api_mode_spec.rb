...
describe API::SorceryController, type: :controller do
  context 'when activated with sorcery' do
    ...
    describe '#login' do
      context 'when succeeds' do
        ...
        it 'dose not set csrf token in session' do
          expect(session[:_csrf_token]).to be_nil
        end
      end
    ...
    it 'require_login before_action saves the url that the user originally wanted' do
      ...
      expect(session[:return_to_url]).to eq 'http://test.host/api/some_action'
    ...
    it 'on successful login the user is redirected to the url he originally wanted' do
      session[:return_to_url] = 'http://test.host/api/some_action'
      post :test_return_to, params: { email: 'bla@bla.com', password: 'secret' }

      expect(response).to redirect_to('http://test.host/api/some_action')
    end
